package com.hotelapp.hotel.repositoryTest;

import com.hotelapp.hotel.model.User;
import com.hotelapp.hotel.repository.UserRepository;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.autoconfigure.orm.jpa.DataJpaTest;
import org.springframework.boot.test.autoconfigure.orm.jpa.TestEntityManager;
import org.springframework.test.context.junit4.SpringRunner;

import javax.transaction.Transactional;

@RunWith(SpringRunner.class)
@DataJpaTest
public class UserRepositoryTest {
    @Autowired
    private TestEntityManager entityManager;
    @Autowired
    private UserRepository userRepository;

    @Test
    public void findByUsernameAndPassword(){
        //given
        User testUser = new User("testuser", "testpassword");
        entityManager.persist(testUser);

        //when
        User foundUser = userRepository.findByUsernameAndPassword(testUser.getUsername(),testUser.getPassword());

        //then
        assert (foundUser.getId()).equals(testUser.getId());
        assert (foundUser.getUsername()).equals(testUser.getUsername());
        assert (foundUser.getPassword()).equals(testUser.getPassword());

        entityManager.flush();
    }
}
